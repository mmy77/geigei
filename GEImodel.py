...
def saveGei():
    rootsavep = '/home/lxq/GeiGait/data/train/'
    model.eval()
    batch_size = 20
    batch_idx = 0
    '''
    while(batch_idx < len(train_loader.dataset)/batch_size):
        ...
        for i in range(len(imgname)):
            ...
            img.save(savepath)
    '''
    rootsavep = '/home/lxq/GeiGait/data/test/'
    batch_idx=0
...
#model.eval()
writer = SummaryWriter(comment='stn-casia-train')
start = time()
#
for epoch in range(1, 1 + 1):
    train(epoch)
# 在一些输入批次中可视化空间转换网络 (STN) 的转换
writer.export_scalars_to_json("./log/stn_casia_train.json")
writer.close()
